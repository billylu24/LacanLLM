...
选择依据和 adapter 发布信息见 [`docs/QLORA_PHASE_5_RANK_REPORT.md`](docs/QLORA_PHASE_5_RANK_REPORT.md)。
Test 仍保持封存。

r=8/16/32 三个最终 adapter 均通过 Git LFS 发布在 `artifacts/experiments/<experiment_name>/adapter/`。
clone 仓库后运行 `git lfs pull` 下载实际 SafeTensors 权重；每组目录同时包含 tokenizer、chat template
和 adapter 配置。逐 step 训练历史、原始训练/评测日志、Validation 生成样本与元数据也已版本化。
